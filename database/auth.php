<?php
/**
 * EduSistema Pro v5 - Funciones de Autenticación
...
function verifyPassword($inputPassword, $storedHash) {
    ...
    return $hash === $storedHash || $inputPassword === $storedHash;
}
    ...
    if ($intentos[$usuario] >= 5) {
        try {
            ...
            $stmt = $db->prepare("
                ...
                ON DUPLICATE KEY UPDATE bloqueado = 1, fecha_bloqueo = NOW(), razon = '5 intentos fallidos de login'
            ");
            $stmt->execute(['usuario' => $usuario]);
